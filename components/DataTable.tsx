...
function StatusBadge({
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span
        className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold ${cls}`}
      >
        {label}
      </span>
      {isLate && (
      ...
      <div className="rounded-xl border border-border bg-white p-4 shadow-soft">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {x.token && (
              <span className="mt-1 inline-flex max-w-full items-center gap-1 rounded-lg bg-primary/10 px-2 py-1 text-[11px] font-bold text-primary">
                <KeyRound className="h-3 w-3 flex-none" />
                <span className="truncate">{x.token}</span>
              </span>
            )}
          </div>
        </div>
        <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
          <div>
            <span className="text-[11px] text-slate-400">Jam Ke-</span>
            <p className="font-medium">{x.period}</p>
          </div>
          <div>
            <span className="text-[11px] text-slate-400">Status</span>
            <p className="mt-0.5">
              <StatusBadge status={x.status} isLate={x.isLate} />
            </p>
          </div>
          <div className="col-span-2">
            <span className="text-[11px] text-slate-400">Keperluan</span>
            <p className="font-medium">{esc(x.purpose)}</p>
          </div>
          <div className="col-span-2">
            <span className="text-[11px] text-slate-400">Waktu</span>
            <p className="font-medium">
              {x.outTime} - {x.deadline}
            </p>
          </div>
        </div>
        {x.status === "pending" && (
          <div className="mt-3 flex gap-2">
            <button
              ...
              className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-green-100 py-2.5 text-xs font-bold text-green-800 active:bg-green-200"
            >
            ...
            <button
              ...
              className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-red-100 py-2.5 text-xs font-bold text-red-800 active:bg-red-200"
            >
    ...
    <>
      <div className="rounded-xl border border-border bg-white p-4 shadow-soft">
        ...
        <div className="mt-2 text-sm">
          ...
          <div className="mt-1">
            <span className="text-[11px] text-slate-400">Keperluan: </span>
            <span className="font-medium">{esc(x.purpose)}</span>
          </div>
          <div className="mt-0.5">
            <span className="text-[11px] text-slate-400">Batas: </span>
            <span className="font-medium">{x.deadline}</span>
          </div>
        </div>
        {x.photo && (
          <div className="mt-2">
        ...
        {x.photoOut && (
          <div className="mt-2">
            ...
            <ClickableImage
              src={x.photoOut}
              alt={`Foto keluar ${x.name}`}
              label={`Foto keluar - ${x.name}`}
              className="mt-1 h-20 w-20 rounded-lg object-cover"
            />
          </div>
        )}
        <div className="mt-3">
          {x.status === "pending" && (
            <button
              onClick={() => onApprove?.(x.id)}
              className="flex w-full items-center justify-center gap-1 rounded-lg bg-green-100 py-2.5 text-xs font-bold text-green-800 active:bg-green-200"
            >
              <Check className="h-3.5 w-3.5" />
              Setujui
            </button>
          ...
          {x.status === "approved" &&
            (allowCapture ? (
              <button
                ...
                className="flex w-full items-center justify-center gap-1 rounded-lg bg-primary py-2.5 text-xs font-bold text-white active:bg-primary-dark"
              >
              ...
              <Link
                ...
                className="flex w-full items-center justify-center gap-1 rounded-lg bg-slate-100 py-2.5 text-xs font-bold text-slate-600 active:bg-slate-200"
              >
                <Eye className="h-3.5 w-3.5" />
                Validasi di Pemantauan
              </Link>
            ))}
          {x.status === "out" &&
            (allowCapture ? (
  ...
  return (
    <div className="rounded-xl border border-border bg-white p-4 shadow-soft">
      ...
      <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
        ...
        <div>
          <span className="text-[11px] text-slate-400">Disetujui</span>
          <p className="font-medium">{x.approvedAt ?? "-"}</p>
        </div>
        <div>
          <span className="text-[11px] text-slate-400">Kembali</span>
          <p className="font-medium">{x.returnedAt ?? "-"}</p>
        </div>
        <div className="col-span-2">
          <StatusBadge status={x.status} isLate={x.isLate} />
        </div>
        ...
        <div className="mt-3 flex gap-3 border-t border-slate-100 pt-3">
          {x.photo && (
            <div>
              <ClickableImage
                src={x.photo}
                alt={`Foto guru piket - ${x.name}`}
                label={`Foto guru piket - ${x.name}`}
          ...
          {x.photoIn && (
            <div>
              <ClickableImage
                src={x.photoIn}
                alt={`Foto masuk - ${x.name}`}
                label={`Foto masuk - ${x.name}`}
                className="h-16 w-16 rounded-lg object-cover"
              />
              <span className="mt-1 block text-[10px] text-slate-400">
                Masuk
...
export default function DataTable({
                  ...
                  <td className="px-3 py-3">
                    {x.status === "pending" && (
                      <div className="flex gap-2">
                        <button
                          onClick={() => onApprove?.(x.id)}
                          className="inline-flex items-center gap-1 rounded-lg bg-green-100 px-3 py-1.5 text-xs font-bold text-green-800 hover:bg-green-200"
                        >
                        ...
                        <button
                          onClick={() => onReject?.(x.id)}
                          className="inline-flex items-center gap-1 rounded-lg bg-red-100 px-3 py-1.5 text-xs font-bold text-red-800 hover:bg-red-200"
                        >
                          <X className="h-3 w-3" />
                          Tolak
                        </button>
                    ...
                    {x.status !== "pending" && (
                      <span className="text-xs text-slate-400">Terpantau</span>
                    )}
                  </td>
                </tr>
              ))}
  ...
  if (view === "guard") {
    const list = permissions.filter((x) =>
      ["pending", "approved", "out"].includes(x.status),
    );
    if (list.length === 0)
      return (
        <div className="py-10 text-center text-sm text-slate-400">
          Tidak ada siswa yang sedang menunggu proses di pos satpam.
        </div>
      );
    return (
      <>
                  ...
                  <td className="px-3 py-3">
                    ...
                    {x.status === "approved" &&
                      (allowCapture ? (
                        <button
                          onClick={() => {
                            setCameraTarget({ id: x.id, mode: "out" });
                            setCameraOpen(true);
                          }}
                        ...
                        <Link
                          href="/dashboard/monitor"
                          className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-200"
                        >
                    ...
                    {x.status === "out" &&
                      (allowCapture ? (
                        <button
                          onClick={() => {
                            setCameraTarget({ id: x.id, mode: "in" });
                            setCameraOpen(true);
                          }}
                          className="inline-flex items-center gap-1 rounded-lg bg-green-100 px-3 py-1.5 text-xs font-bold text-green-800 hover:bg-green-200"
                        >
  ...
  return (
    <>
      <div className="flex flex-col gap-3 lg:hidden">
        {permissions.map((x) => (
          <HistoryMobileCard key={x.id} x={x} />
        ))}
      </div>
      <div className="hidden overflow-x-auto lg:block">
